...
export default function AuthScreen() {
  ...
  const { oauth_error: oauthErrorParam } = useLocalSearchParams<{ oauth_error?: string | string[] }>();
  ...
  useEffect(() => {
    ...
    void import("@/data/api/oauth").then((m) => m.fetchOAuthProviders()).then(setOauthProviders);
  ...
  useEffect(() => {
    ...
    const oauthError = typeof oauthErrorParam === "string"
    ...
    if (oauthError !== "account_not_found") return;
  ...
  }, [oauthErrorParam, router, showSnackbar]);
  ...
  const firstNameValid = mode === "login" || firstName.trim().length >= 2;

  const legalOk = mode === "login" || acceptLegal;

  const canSubmit = emailValid && passwordValid && firstNameValid && legalOk && !loading;
  ...
  const onOAuth = async (provider: OAuthProvider) => {
    ...
    if (mode === "register" && !acceptLegal) {
      ...
      showSnackbar("Accepte les conditions pour t'inscrire.");
          ...
          <View style={styles.card}>
            ...
            {mode === "register" ? (
              ...
              <TextField
                ...
                error={firstName.length > 0 && !firstNameValid ? "Prénom trop court." : undefined}
