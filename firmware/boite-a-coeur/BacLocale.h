...
struct BacLocale {
  ...
  static void prepare(const char *locale) {
    ...
    copy(lbl_back, sizeof(lbl_back),
         pick(loc, "Retour", "Back", "Volver", "Voltar", "Indietro", "Zuruck"));
    copy(lbl_confirm, sizeof(lbl_confirm),
         pick(loc, "Confirmer", "Confirm", "Confirmar", "Confirmar", "Conferma",
              "Bestatigen"));
    copy(lbl_settings, sizeof(lbl_settings),
         pick(loc, "Paramètres", "Settings", "Ajustes", "Definicoes",
              "Impostazioni", "Einstellungen"));
    copy(lbl_reset, sizeof(lbl_reset),
         pick(loc, "Réinitialiser", "Reset", "Restablecer", "Repor",
              "Reimposta", "Zurucksetzen"));
    copy(lbl_info, sizeof(lbl_info),
         pick(loc, "Informations", "Information", "Informacion", "Informacao",
              "Informazioni", "Informationen"));
    copy(lbl_quit, sizeof(lbl_quit),
         pick(loc, "Quitter", "Exit", "Salir", "Sair", "Esci", "Beenden"));
    copy(lbl_wifi, sizeof(lbl_wifi), "WiFi");
    copy(lbl_language, sizeof(lbl_language),
    ...
    copy(lost_title, sizeof(lost_title),
    ...
    copy(lost_l3, sizeof(lost_l3),
         pick(loc, "pas à se connecter", "connect to the internet",
              "conectarse a internet", "ligar-se a internet",
              "connettersi a internet", "mit dem Internet verbinden"));
    copy(lost_l4, sizeof(lost_l4),
         pick(loc, "à internet", "right now", "ahora", "agora", "adesso",
              "verbinden"));
    copy(lost_l5, sizeof(lost_l5),
         pick(loc, "Vérifiez la configuration", "Check settings in the app",
    ...
    copy(splash_boot, sizeof(splash_boot),
         pick(loc, "Démarrage...", "Starting...", "Iniciando...",
              "A iniciar...", "Avvio...", "Start..."));
    copy(welcome_l1, sizeof(welcome_l1),
    ...
    copy(next, sizeof(next),
         pick(loc, "Suivant", "Next", "Siguiente", "Seguinte", "Avanti",
              "Weiter"));
    copy(new_message, sizeof(new_message),
         pick(loc, "Nouveau message", "New message", "Nuevo mensaje",
              "Nova mensagem", "Nuovo messaggio", "Neue Nachricht"));
    copy(new_message_ephemeral, sizeof(new_message_ephemeral),
         pick(loc, "Message \xC3\xA9ph\xC3\xA9m\xC3\xA8re", "Ephemeral message",
              "Mensaje ef\xC3\xADmero", "Mensagem ef\xC3\xA9mera",
              "Messaggio efimero", "Fl\xC3\xBCchtige Nachricht"));
    copy(ephemeral_open, sizeof(ephemeral_open),
         pick(loc, "10 sec · Ouvrir", "10 sec · Open", "10 seg · Abrir",
              "10 seg · Abrir", "10 sec · Apri", "10 Sek · Offnen"));
    copy(open_msg, sizeof(open_msg),
         pick(loc, "Ouvrir", "Open", "Abrir", "Abrir", "Apri", "Offnen"));
    copy(dl_app, sizeof(dl_app),
         pick(loc, "Téléchargez l'application", "Download the app",
              "Descarga la aplicacion", "Transfere a aplicacao",
              "Scarica l'app", "Lade die App herunter"));
    copy(dl_app2, sizeof(dl_app2),
         pick(loc, "pour configurer votre boîte", "to set up your Lovebox",
              "para configurar tu caja", "para configurar a caixa",
              "per configurare la scatola", "um deine Box einzurichten"));
    copy(bt_l1, sizeof(bt_l1),
         pick(loc, "Activez le Bluetooth", "Turn on Bluetooth",
              "Activa el Bluetooth", "Ativa o Bluetooth", "Attiva il Bluetooth",
              "Bluetooth einschalten"));
    copy(bt_l2, sizeof(bt_l2),
         pick(loc, "sur votre smartphone", "on your phone", "en tu telefono",
              "no telemovel", "sul telefono", "auf dem Smartphone"));
    copy(bt_l3, sizeof(bt_l3),
         pick(loc, "et connectez-vous à", "and connect to", "y conectate a",
              "e liga-te a", "e connettiti a", "und verbinde dich mit"));
    copy(bt_l4, sizeof(bt_l4),
         pick(loc, "Poursuivez sur le smartphone", "Continue on your phone",
              "Continua en el telefono", "Continua no telemovel",
              "Continua sul telefono", "Weiter auf dem Smartphone"));
    copy(bravo, sizeof(bravo),
    ...
    copy(wifi_err_title, sizeof(wifi_err_title),
         pick(loc, "Connexion impossible à", "Cannot connect to",
              "No se puede conectar a", "Nao foi possivel ligar a",
              "Impossibile connettersi a", "Verbindung fehlgeschlagen mit"));
    copy(wifi_err_hint, sizeof(wifi_err_hint),
         pick(loc, "Vérifiez vos paramètres", "Check your settings",
              "Revisa los ajustes", "Verifica as definicoes",
              "Controlla le impostazioni", "Prufe die Einstellungen"));
    copy(wifi_conn_title, sizeof(wifi_conn_title),
         pick(loc, "Connexion au WiFi", "Connecting to WiFi",
              "Conectando al WiFi", "A ligar ao WiFi", "Connessione WiFi",
              "WiFi-Verbindung"));
    copy(wifi_conn_progress, sizeof(wifi_conn_progress),
         pick(loc, "en cours...", "in progress...", "en curso...", "a ligar...",
              "in corso...", "lauft..."));
    copy(idle_no_msg, sizeof(idle_no_msg),
         pick(loc, "Pas de nouveau message", "No new message",
    ...
    copy(disc_done, sizeof(disc_done),
         pick(loc, "Boîte déconnectée", "Lovebox disconnected",
              "Caja desconectada", "Caixa desligada", "Scatola disconnessa",
              "Box getrennt"));
    copy(disc_need1, sizeof(disc_need1),
         pick(loc, "Pour utiliser la boîte,", "To use the Lovebox,",
              "Para usar la caja,", "Para usar a caixa,",
              "Per usare la scatola,", "Um die Box zu nutzen,"));
    copy(disc_need2, sizeof(disc_need2),
         pick(loc, "vous devez configurer", "you need an internet",
              "necesitas una", "precisas de uma", "serve una connessione",
              "brauchst du eine"));
    copy(disc_need3, sizeof(disc_need3),
         pick(loc, "une connexion internet.", "connection.",
              "conexion a internet.", "ligacao a internet.", "internet.",
              "Internetverbindung."));
    copy(wifi_test_progress, sizeof(wifi_test_progress),
         pick(loc, "Test de connexion...", "Testing connection...",
              "Probando conexion...", "A testar ligacao...",
              "Test connessione...", "Verbindungstest..."));
    copy(wifi_test_ok, sizeof(wifi_test_ok),
         pick(loc, "Connexion parfaite !", "Connection OK!",
              "Conexion perfecta!", "Ligacao perfeita!", "Connessione OK!",
              "Verbindung OK!"));
    copy(wifi_test_fail1, sizeof(wifi_test_fail1),
         pick(loc, "Serveurs injoignables", "Cannot reach servers",
              "Servidores inaccesibles", "Servidores inacessiveis",
              "Server non raggiungibili", "Server nicht erreichbar"));
    copy(wifi_test_fail2, sizeof(wifi_test_fail2),
         pick(loc, "Réessayez plus tard", "Try again later",
              "Intentalo mas tarde", "Tenta mais tarde", "Riprova piu tardi",
              "Spater erneut versuchen"));
    copy(info_fw, sizeof(info_fw),
         pick(loc, "Version FW:", "FW version:", "Version FW:", "Versao FW:",
              "Versione FW:", "FW-Version:"));
    copy(info_build, sizeof(info_build),
         pick(loc, "Build:", "Build:", "Build:", "Build:", "Build:", "Build:"));
    copy(info_model, sizeof(info_model),
         pick(loc, "Modèle:", "Model:", "Modelo:", "Modelo:", "Modello:",
              "Modell:"));
    copy(info_mac, sizeof(info_mac),
         pick(loc, "MAC:", "MAC:", "MAC:", "MAC:", "MAC:", "MAC:"));
    copy(factory_q1, sizeof(factory_q1),
         pick(loc, "Réinitialiser la boîte ?", "Reset the Lovebox?",
    ...
    copy(factory_q3, sizeof(factory_q3),
         pick(loc, "affichera le setup", "the setup wizard",
              "mostrara el setup", "mostra o setup", "mostrera il setup",
              "erscheint das Setup"));
    copy(factory_q4, sizeof(factory_q4),
         pick(loc, "initial.", "again.", "inicial.", "inicial.", "iniziale.",
              "erneut."));
    copy(factory_progress, sizeof(factory_progress),
    ...
    copy(ota_warn, sizeof(ota_warn),
         pick(loc, "Ne pas débrancher", "Do not unplug", "No desenchufar",
              "Nao desligar", "Non scollegare", "Nicht trennen"));
    copy(ota_failed, sizeof(ota_failed),
  ...
  static const char *dayName(int wday) {
    static const char *days_fr[] = {"Dimanche", "Lundi",    "Mardi", "Mercredi",
                                    "Jeudi",    "Vendredi", "Samedi"};
    static const char *days_en[] = {"Sunday",    "Monday",   "Tuesday",
                                    "Wednesday", "Thursday", "Friday",
                                    "Saturday"};
    static const char *days_es[] = {"Domingo", "Lunes",   "Martes", "Miercoles",
                                    "Jueves",  "Viernes", "Sabado"};
    static const char *days_pt[] = {"Domingo", "Segunda", "Terca", "Quarta",
                                    "Quinta",  "Sexta",   "Sabado"};
    static const char *days_it[] = {"Domenica",  "Lunedi",  "Martedi",
                                    "Mercoledi", "Giovedi", "Venerdi",
                                    "Sabato"};
    static const char *days_de[] = {"Sonntag",  "Montag",     "Dienstag",
  ...
  static const char *monthName(int mon) {
    ...
    if (mon < 0 || mon > 11)
      mon = 0;
    if (strcmp(_loc, "en") == 0)
      return months_en[mon];
    if (strcmp(_loc, "es") == 0)
      return months_es[mon];
    if (strcmp(_loc, "pt") == 0)
      return months_pt[mon];
    if (strcmp(_loc, "it") == 0)
      return months_it[mon];
    if (strcmp(_loc, "de") == 0)
      return months_de[mon];
...
char BacLocale::lbl_settings[20] = "Paramètres";
char BacLocale::lbl_reset[20] = "Réinitialiser";
...
char BacLocale::lbl_disconnect[20] = "Déconnecter";
